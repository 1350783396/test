        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (f.VerifySignature(result, SignatureByte))
            {
                ...
                if (payResult == "10")
                {
                    ...
                    NJiaSu.Libraries.LogHelper.LogResult("99bill", "payResult=" + payResult + string.Format(",{0}将更新订单状态。", orderId));
                    //此处做商户逻辑处理
                    string msg = ETicket.BLL.OrderSheetBLL.Instance.PayOnline(orderId, ETicket.Utility.OnlinePayEnum.快钱.ToString(), "", "", dealId);
                    if (msg != "")
                        ETicket.BLL.OrderSheetBLL.Instance.PayLogUpdateCallBackNO(orderId, "", dealId);
                    ...
                    Response.Write("<result>1</result>" + "<redirecturl></redirecturl>");
                }
                else
                {
                    //以下是我们快钱设置的show页面，商户需要自己定义该页面。
                    //Response.Write("<result>1</result>" + "<redirecturl>http://219.233.173.50:8804/dongjian/rmb/show.aspx?msg=error</redirecturl>");
                    NJiaSu.Libraries.LogHelper.LogResult("99bill","payResult=" + payResult+",没有更新订单状态");
                }
            }
            else
            {
                ...
                NJiaSu.Libraries.LogHelper.LogResult("99bill", "signMsgVal=" + "(" + signMsgVal + ")" + "---" + "signMsg =" + signMsg + "---错误");
            }
        }
