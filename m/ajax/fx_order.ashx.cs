...
namespace ETicket.Web.ajax
{
    ...
    /// fx_order 的摘要说明
    /// </summary>
    public class fx_order : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            int validProductID=0, validUserID=0;
            var validMsg = wap.WapHelper.GetUserID_ProductID("q",out validUserID, out validProductID);
            if(validMsg!="")
            {
                //核对签名不通过
            }
            ...
            */
            EFEntity.User user = BLL.UserBLL.Instance.GetEntity(p => p.UserID == validUserID);

            //返回结果
            api_h.JsonResult<api_h.Order> jsonModel = new api_h.JsonResult<api_h.Order>();
            string jsonStr = "";
            ...
            {
                jsonModel.Status = "0";
                jsonModel.Msg = "签名和提交的信息不对称，无法提交订单。";
                jsonStr = JsonHelper.GetJson<api_h.JsonResult<api_h.Order>>(jsonModel);
            ...
            try
            {
                ...
                */
                ...
                if (fxPrice != null)
                {
                    myPrice = fxPrice.MyPrice.Value;
                }
                else
                {
                    myPrice = suk.FXPrice_Recommend.Value;
                }
                decimal fxTotalPrice = Math.Round(buyNum * myPrice, 2);
                if(fxTotalPrice<=0)
                {
                    trans.RollBack();
                    ...
                    //生成json
                    jsonModel.Status = "0";
                    jsonModel.Msg = "对不起，产品价格异常，无法购买。";
                    jsonStr = JsonHelper.GetJson<api_h.JsonResult<api_h.Order>>(jsonModel);
                    context.Response.Write(jsonStr);
                    return;
                }
                var fxReturn = fxTotalPrice - totalPrice;
                if(fxReturn<=0)
                {
                    //生成json
                    jsonModel.Status = "0";
                ...
                //记录分销价格及返还多少
                order.FxUnitPrice = myPrice;
                order.FxTotalPrice = fxTotalPrice;
                order.FxReturnPrice = fxReturn;
                ...
                order.ClientType = ETicket.Utility.ClientTypeEnum.weixin.ToString();
            ...
            if (transState)
            {
                ...
                */
                ...
                List<api_h.Order> listOrder = new List<api_h.Order>();
                var itemResult = api_h.ApiHelper.OrderSheet2Result(updateModel, user, false);
                itemResult.SheetID = "";//不返回
